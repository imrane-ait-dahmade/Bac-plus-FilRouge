            ...
            <div class="grid grid-cols-1 md:grid-cols-3">
                ...
                <div class="md:col-span-2 p-6 space-y-6 border-r border-gray-200">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                        ...
                        @else
                            <div class="w-16 h-16 bg-gray-100 rounded-md flex items-center justify-center flex-shrink-0 border border-gray-200">
                        ...
                        <div>
                            ...
                            @endif
                            <span class="mt-1 inline-block px-2 py-0.5 text-xs font-medium rounded-full {{ $etablissement->typeecole == 'Public' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                    ...
                    <div>
                        ...
                        <div class="space-y-2 text-sm text-gray-600">
                            <div class="flex items-start"> {{-- Use items-start for long addresses --}}
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 text-gray-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                                <span>{{ $etablissement->adresseetablissement ?? 'Adresse non spécifiée' }}</span>
                            </div>
                            <div class="flex items-center">
                                ...
                                </svg> {{-- Changed Icon to represent city/building --}}
                                <span>{{ $etablissement->villeetablissement ?? 'Ville non spécifiée' }}</span>
                            </div>
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /> {{-- Changed icon to map/region --}}
                                </svg>
                                <span>Région: {{ $etablissement->region->nom_region ?? $etablissement->region_id ?? 'N/A' }}</span> {{-- Adjust if region relationship is different --}}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600 leading-relaxed prose max-w-none"> {{-- Added prose for better text formatting if needed --}}
                            {!! nl2br(e($etablissement->descirptionetablissement ?? 'Aucune description fournie.')) !!} {{-- Use nl2br for line breaks --}}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                                ...
                                <div class="text-gray-800">{{ $etablissement->universite ?? 'N/A' }}</div>
                            </div>
                            <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                            ...
                            <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 sm:col-span-2">
                                ...
                                <div class="text-gray-800">{{ $etablissement->nombreetudiant ? number_format($etablissement->nombreetudiant, 0, ',', ' ') : 'N/A' }}</div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-3">
                            ...
                            @if($etablissement->relationLoaded('filieres') && $etablissement->filieres->count() > 0)
                                @foreach($etablissement->filieres as $filiere)
                                    <div class="p-3 border border-gray-200 rounded-lg bg-white hover:shadow-sm transition duration-150">
                                        <div class="flex justify-between items-start gap-2">
                                            <h4 class="font-medium text-gray-800 text-base">{{ $filiere->nom_filiere ?? 'Nom non disponible' }}</h4>
                                            {{-- Optional: Link to filiere detail page if you have one --}}
                                            {{-- <a href="{{ route('filiere.show', $filiere->id) }}" class="text-xs text-custom-primary hover:underline whitespace-nowrap font-medium">Voir détails</a> --}}
                                        </div>
                                        <div class="mt-1 text-sm text-gray-600 flex flex-wrap gap-x-4 gap-y-1">
                                            @if($filiere->niveau) {{-- Assuming 'niveau' field exists --}}
                                            <span class="flex items-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1 text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 00-5.657 0 3.066 3.066 0 00-1.83 2.278.75.75 0 001.038.802 1.567 1.567 0 012.33 0 .75.75 0 001.037-.802 3.066 3.066 0 00-1.83-2.278zM12.267 3.455a3.066 3.066 0 00-5.657 0 3.066 3.066 0 00-1.83 2.278.75.75 0 001.038.802 1.567 1.567 0 012.33 0 .75.75 0 001.037-.802 3.066 3.066 0 00-1.83-2.278zM18.267 3.455a3.066 3.066 0 00-5.657 0 3.066 3.066 0 00-1.83 2.278.75.75 0 001.038.802 1.567 1.567 0 012.33 0 .75.75 0 001.037-.802 3.066 3.066 0 00-1.83-2.278zM6.267 10.455a3.066 3.066 0 00-5.657 0 3.066 3.066 0 00-1.83 2.278.75.75 0 001.038.802 1.567 1.567 0 012.33 0 .75.75 0 001.037-.802 3.066 3.066 0 00-1.83-2.278zM12.267 10.455a3.066 3.066 0 00-5.657 0 3.066 3.066 0 00-1.83 2.278.75.75 0 001.038.802 1.567 1.567 0 012.33 0 .75.75 0 001.037-.802 3.066 3.066 0 00-1.83-2.278zM18.267 10.455a3.066 3.066 0 00-5.657 0 3.066 3.066 0 00-1.83 2.278.75.75 0 001.038.802 1.567 1.567 0 012.33 0 .75.75 0 001.037-.802 3.066 3.066 0 00-1.83-2.278z" clip-rule="evenodd" /></svg>
                                                Niveau: {{ $filiere->niveau }}
                                            </span>
                                            @endif
                                            @if($filiere->duree) {{-- Assuming 'duree' field exists --}}
                                            <span class="flex items-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1 text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" /></svg>
                                                Durée: {{ $filiere->duree }}
                                            </span>
                                            @endif
                                            ...
                                            {{-- @if($filiere->domaine)
                                               <span class="flex items-center">
                                                   <svg>...</svg> Domaine: {{ $filiere->domaine->nom ?? 'N/A' }}
                                               </span>
                                            @endif --}}
                                        </div>
                                        @if($filiere->description_filiere) {{-- Assuming 'description_filiere' field exists --}}
                                        <p class="mt-2 text-xs text-gray-500 leading-relaxed line-clamp-2">{{ $filiere->description_filiere }}</p>
                                        @endif
                                    </div>
                                @endforeach
                            @else
                    ...
                    {{-- Metadata --}}
                    <div class="pt-4 border-t border-gray-200 text-xs text-gray-400 mt-auto"> {{-- Added mt-auto in case column is short --}}
                ...
                <div class="p-6 bg-gray-50 space-y-6">
                    ...
                    <div>
                        ...
                        @if($etablissement->image)
                            <img src="{{ asset('Images/PhotoEcoles/' . $etablissement->image) }}"
                                 ...
                                 class="w-full h-auto rounded-lg shadow-md border border-gray-200 object-cover max-h-64"> {{-- Added max-h --}}
                        @else
                            <div class="w-full h-48 bg-gray-200 rounded-lg flex items-center justify-center text-gray-400 border border-gray-300">
                        ...
                        <div class="space-y-2 text-sm">
                            @if($etablissement->telephone)
                                <div class="flex items-center text-gray-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 flex-shrink-0 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                    <a href="tel:{{ $etablissement->telephone }}" class="break-all hover:text-custom-primary hover:underline">{{ $etablissement->telephone }}</a>
                                </div>
                            @endif
                            ...
                            @if($etablissement->site_web)
                                <div class="flex items-start text-gray-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 flex-shrink-0 text-gray-400 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>
                                    <a href="{{ Str::startsWith($etablissement->site_web, ['http://', 'https://']) ? $etablissement->site_web : 'http://' . $etablissement->site_web }}" target="_blank" rel="noopener noreferrer" class="break-all hover:text-custom-primary hover:underline">Site Web</a>
                                </div>
                            @endif
                            @if($etablissement->site_inscription)
                                <div class="flex items-start text-gray-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 flex-shrink-0 text-gray-400 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                                    <a href="{{ Str::startsWith($etablissement->site_inscription, ['http://', 'https://']) ? $etablissement->site_inscription : 'http://' . $etablissement->site_inscription }}" target="_blank" rel="noopener noreferrer" class="break-all hover:text-custom-primary hover:underline">Lien d'Inscription</a>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    {{-- Social Media --}}
                    @if($etablissement->facebook || $etablissement->instagram || $etablissement->linkedin || $etablissement->twitter) {{-- Added Twitter Example --}}
                    <div>
                        <h3 class="text-lg font-semibold text-gray-700 mb-3 border-b pb-1">Réseaux Sociaux</h3>
                        <div class="flex flex-wrap gap-3"> {{-- Use gap for spacing --}}
                            @if($etablissement->facebook)
                                <a href="{{ Str::startsWith($etablissement->facebook, ['http://', 'https://']) ? $etablissement->facebook : 'https://facebook.com/' . $etablissement->facebook }}" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-blue-600 transition duration-150" title="Facebook">
                                    <span class="sr-only">Facebook</span> {{-- Accessibility --}}
                                    <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fill-rule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clip-rule="evenodd" /></svg>
                                </a>
                            @endif
                            @if($etablissement->instagram)
                                <a href="{{ Str::startsWith($etablissement->instagram, ['http://', 'https://']) ? $etablissement->instagram : 'https://instagram.com/' . $etablissement->instagram }}" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-pink-600 transition duration-150" title="Instagram">
                            ...
                            @if($etablissement->linkedin)
                                <a href="{{ Str::startsWith($etablissement->linkedin, ['http://', 'https://']) ? $etablissement->linkedin : 'https://linkedin.com/company/' . $etablissement->linkedin }}" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-blue-700 transition duration-150" title="LinkedIn">
                            ...
                            {{-- @if($etablissement->twitter)
                                <a href="{{ Str::startsWith($etablissement->twitter, ['http://', 'https://']) ? $etablissement->twitter : 'https://twitter.com/' . $etablissement->twitter }}" target="_blank" rel="noopener noreferrer" class="text-gray-500 hover:text-black transition duration-150" title="Twitter / X">
...
@push('styles')
    ...
    <style>
        ...
        /* Added prose styles for better description formatting */
        .prose {
        ...
        /* Add other prose styles if needed (ul, ol, blockquote...) */
...
{{-- Add push for scripts if needed --}}
{{-- @push('scripts')
    <script>
        // Add any specific JS for this page if required
    </script>
@endpush --}}
