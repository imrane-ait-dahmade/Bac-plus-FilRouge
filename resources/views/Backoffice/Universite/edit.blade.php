...
<div class="max-w-4xl mx-auto p-6">
    <div class="bg-white shadow-sm rounded-lg">
        <div class="p-6">
            <h2 class="text-xl font-semibold mb-6">Modifier l'Université</h2>
            ...
            <form action="{{ route('universites.update', $universite) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nom" id="nom"
                               ...
                               value="{{ old('nom', $universite->nom) }}" required>
                        @error('nom')
                    ...
                    <div>
                        ...
                        <input type="text" name="directeur" id="directeur"
                               ...
                               value="{{ old('directeur', $universite->directeur) }}" required>
                        @error('directeur')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <select name="region_id" id="region_id"
                            ...
                            @foreach($regions as $region)
                                <option value="{{ $region->id }}" 
                                    {{ old('region_id', $universite->region_id) == $region->id ? 'selected' : '' }}>
                                    {{ $region->nom }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="flex gap-6">
                            <div class="flex items-center space-x-2">
                                <input type="radio" name="type" id="type_public" value="public"
                                    ...
                                    {{ old('type', $universite->type) == 'public' ? 'checked' : '' }}>
                            ...
                            <div class="flex items-center space-x-2">
                                <input type="radio" name="type" id="type_prive" value="prive"
                                       class="h-4 w-4 border-gray-300 text-custom-primary focus:ring-custom-primary"
                                    {{ old('type', $universite->type) == 'prive' ? 'checked' : '' }}>
